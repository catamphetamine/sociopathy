...
function initialize_join_dialog()
{
	...
	join_dialog.bind('open', function()
	{
		...
		Режим.помощь('Выберите себе имя в нашей сети. Например, "Иван Петрович Сидоров".')
	})
	
	join_dialog.bind('close', function()
	{
		if (!прописан)
			Режим.помощь(Режим.возстановить_помощь('вне_окошка'))
	})
	
	join_form_slider.bind('slide_No_2', function()
	{
		Режим.помощь('Нажмите на картинку, соответствующую вашему полу.')
	})
	
	join_form_slider.bind('slide_No_3', function()
	{
		Режим.помощь('Разскажите нам, откуда вы. Например: "Москва", "Где-то на границе с Монголией", "Кольский полуостров".')
	})
	
	join_form_slider.bind('slide_No_4', function()
	{
		Режим.помощь('По этому паролю вы будете входить в нашу сеть. Например: "белый слон жуёт морковь", "кто не спрятался - я не виноват", "у меня везде один пароль".')
	})
...
{
	Режим.помощь('Это заглавная страница нашей сети. Воспользуйтесь меню слева сверху для перехода в какой-либо раздел сети.')
	...
	Ajax.get('/приложение/приглашение/проверить', { приглашение: приглашение },
	{
		error: function(message)
		{
			if (message === "Нет такого приглашения в списке")
				Режим.помощь('Ваше приглашение не найдено в списке. Возможно кто-то уже прописался по нему. Возможно эта ссылка неправильная. Напишите нам об этом, и мы вам поможем.')
			else
				Режим.помощь('Произошла ошибка в ходе проверки вашего приглашения. Напишите нам об этом, и мы вам поможем.')
		...
		ok: function(данные)
		{
			...
			Режим.помощь('Теперь вы можете прописаться, нажав на кнопку "Присоединиться" внизу страницы.')
			callback()
		}
	})
}
...
function join_submission(data)
{
	...
	{ 
		error: function()
		{
			...
			Режим.помощь('Произошла какая-то ошибка. Напишите нам об этом, и мы вам поможем.')
		}, 
		ok: function(данные) 
		{ 
			прописан = true
			Режим.помощь('Сейчас страница будет перезагружена.')
