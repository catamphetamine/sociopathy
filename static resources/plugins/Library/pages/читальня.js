(function()
{
	...
	page.load = function()
	{
		...
		if (page.data.раздел)
			page.подсказка('написание заметок читальни', 'Вы можете написать заметку, выбрав действие «Написать заметку» (клавиша «' + Настройки.Клавиши.Показать_действия + '»)')
	...
	function categories_loaded()
	{
		...
		{
			page.подсказка('удаление разделов и заметок читальни2', 'Для того, чтобы удалить раздел (или заметку), просто «выбросите» его (или её) мышью со страницы, и после этого «сохраните правки»')
