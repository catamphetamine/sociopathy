...
http.post '/управление/хранилище/изменить', (ввод, вывод, пользователь) ->
	...
	цепь(вывод)
		...
		.сделать ->
			...
			db('people').update({ _id: человек._id}, data, @)
		
		.сделать ->
			console.log(второй_человек)
			db('people').update({ _id: второй_человек._id}, { $set: { полномочия: ['приглашения'] } }, @)
