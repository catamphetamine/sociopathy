...
http.post '/хранилище/изменить', (ввод, вывод) ->
	...
	цепь(вывод)
		...
		.сделать ->
			хранилище.collection('talks').save({ id: 'Привет, 12.05.2012 в 18:00, Дождь со Снегом', название: 'Привет', участники: [человек._id], создана: new Date() }, @)
				
		.сделать (talk) ->
			беседа = talk
			хранилище.collection('talks').save({ id: 'Отпуск, 12.05.2012 в 18:00, Дождь со Снегом', название: 'Отпуск', участники: [человек._id], создана: new Date() }, @)
		...
		.сделать ->
			хранилище.collection('discussions').save({ id: 'Есть ли жизнь на Марсе?, 12.05.2012 в 18:00, Дождь со Снегом', название: 'Есть ли жизнь на Марсе?', участники: [человек._id], создано: new Date() }, @)
				
		.сделать (discussion) ->
			обсуждение = discussion
			хранилище.collection('discussions').save({ id: 'Кто играет в Доту?, 12.05.2012 в 18:00, Дождь со Снегом', название: 'Кто играет в Доту?', участники: [человек._id], создано: new Date() }, @)
		...
		.сделать ->
			messages =
			[
				{ сообщение: 'Раз два три' },
				{ сообщение: 'Четыре <b>пять</b> шесть' },
				{ сообщение: 'И ещё сообщение' }
			]
			
			@.done(messages)

		.все_вместе (message) ->
			message.отправитель = человек._id
		...
		.сделать ->
			...
			[
				{ сообщение: 'Есть ли' },
				{ сообщение: 'Никто не знает <b>того</b>' },
				{ сообщение: 'Марсоходы' }
			]
