...
http.post '/хранилище/изменить', (ввод, вывод) ->
	...
	цепь(вывод)
		...
		.сделать ->
			хранилище.collection('trash').save({ пользователь: человек._id, _id: { test: 3456 }, что: 'фотография', содержимое: { url: '...', title: 'Проверка' }, когда_выброшено: new Date() }, @)
		
		.сделать ->
			хранилище.collection('trash').save({ пользователь: человек._id, _id: { test: 345 }, что: 'заметка', содержимое: { title: 'Любая заметка', content: 'Бла бла бла' }, когда_выброшено: new Date() }, @)
...
#			хранилище.collection('people').ensureIndex 'почта', true, @
		...
		.сделать ->
			хранилище.collection('diaries').ensureIndex 'пользователь', yes, @
		...
		.сделать ->
			хранилище.collection('journals').ensureIndex 'пользователь', yes, @
