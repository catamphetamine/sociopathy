...
http.post '/управление/хранилище/изменить', (ввод, вывод, пользователь) ->
	...
	цепь(вывод)
		...
		.сделать ->
			db('library_article_drafts').drop @
		...
		.сделать ->
			db('library_article_drafts').ensureIndex 'заметка', yes, @
			
		.сделать ->
			db('library_article_drafts').ensureIndex 'пользователь', yes, @
...
#			db('people').ensureIndex 'почта', true, @
		...
		# заполнить черновики заметок
		
		#.сделать ->
		#	db('library_article_drafts').save({ заметка: заметка._id, пользователь: человек._id }, @)
