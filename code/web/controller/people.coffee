...
http.get '/человек/картинки/альбомы', (ввод, вывод) ->
	цепь(вывод)
		...
		.сделать (человек) ->
			хранилище.collection('pictures').find({ пользователь: человек._id }, { sort: [['_id', 1]] }).toArray(@)
		.сделать (альбомы) ->
...
http.get '/человек/видео/альбомы', (ввод, вывод) ->
	цепь(вывод)
		.сделать ->
			пользовательское.взять({ 'адресное имя': ввод.настройки['адресное имя'] }, @)
		.сделать (человек) ->
			хранилище.collection('videos').find({ пользователь: человек._id }, { sort: [['_id', 1]] }).toArray(@)
		.сделать (альбомы) ->
			for альбом in альбомы
				альбом.видео = []
			вывод.send(альбомы: альбомы)
