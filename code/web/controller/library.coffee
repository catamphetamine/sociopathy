...
http.post "/получить_право_на_правку_заметки", (ввод, вывод) ->
	...
	цепь(вывод)
		...
		.сделать (заметка) ->
			...
			db('library_articles').update({ _id: _id }, { $set: { 'кто правит': ввод.session.пользователь._id }}, @)
		.сделать ->
			db('library_articles').findOne({ _id: _id }, @)
		.сделать (заметка) ->
			if заметка['кто правит'].toString() != ввод.session.пользователь._id.toString()
...
http['delete'] '/черновик', (ввод, вывод) ->
	...
	цепь(вывод)
		...
		.сделать (заметка) ->
			db('library_article_drafts').remove({ '_id заметки': _id, '_id человека': ввод.session.пользователь._id }, @)
		.сделать ->
			вывод.send @.$.заметка