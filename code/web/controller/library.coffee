...
http.get "/раздел или заметка", (ввод, вывод) ->
	цепь(вывод)
		.сделать ->
			db('library_paths').findOne({ путь: ввод.данные.путь }, @)
			
		.сделать (раздел_или_заметка) ->
			if not раздел_или_заметка?
				return @.error("Раздел или заметка «#{ввод.данные.путь}» не найдены", { уровень: 'ничего страшного' })
...
http.get '/читальня/раздел или заметка/найти', (ввод, вывод) ->
	...
	цепь(вывод)
		...
		.сделать ->
			db('library_categories').find({ название: шаблон }, { limit: ввод.данные.сколько }).toArray @._.в 'разделы'
			
		.сделать ->
			Object.выбрать(['_id', 'id', 'название'], @._.разделы)
			if (@._.разделы.length == ввод.данные.сколько)
				return вывод.send(разделы: @._.разделы)
			db('library_articles').find({ название: шаблон }, { limit: ввод.данные.сколько }).toArray @._.в 'заметки'
