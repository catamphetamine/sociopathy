...
http.post '/сеть/читальня/раздел', (ввод, вывод, пользователь) ->
	...
	for раздел in разделы.новые
		...
		разделы.add(db('library_categories')._.find_one(data))
		
	if not разделы.пусто()
		throw 'Раздел с таким именем уже есть в этом надразделе: ' + разделы[0]
	...
	for раздел in разделы.новые
		...
		новые_разделы.add(db('library_categories')._.save(data, { safe: yes }))
	...
	result = 
		...
		удалённые_разделы: JSON.stringify(удалённые_разделы )
...
http.put '/сеть/читальня/заметка', (ввод, вывод, пользователь) ->
	...
	заметка = db('library_articles')._.save({ название: название, содержимое: содержимое, раздел: раздел }, { safe: yes })
