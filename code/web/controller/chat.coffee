		...
		соединение.on 'пользователь', (тайный_ключ) ->
			...
			цепь_websocket(соединение)
				...
				.сделать (who_is_online) ->
					...
					соединение.broadcast.emit('user_online', пользовательское.поля(пользователь))
					...
					соединение.on 'сообщение', (сообщение) ->
						...
						#данные_сообщения =
						#	отправитель: пользовательское.выбрать(пользователь)
						#	сообщение: сообщение
						#	время: new Date()
					...
					выход = ->
						...
						цепь(websocket)
							...
							.сделать () ->
								соединение.broadcast.emit('offline', Object.выбрать(['_id'], пользователь))
	...
	цепь(вывод)
		...
		.сделать (отправители) ->
			@.$.сообщения.forEach (сообщение, индекс) ->
				сообщение.отправитель = пользовательское.поля(отправители[индекс])
			@()
