	...
	.on 'connection', (соединение) ->
		соединение.on 'пользователь', (тайный_ключ) ->
			...
			соединение.on 'не смотрит', () ->
				соединение.broadcast.emit('не смотрит', Object.выбрать_поля(['_id'], пользователь))
			...
			соединение.on 'сообщение', (сообщение) ->
				...
				#данные_сообщения =
				#	отправитель: Object.выбрать_поля(['адресное имя', 'имя', '_id'], пользователь)
				#	сообщение: сообщение
				#	время: new Date()
				#return болталка.emit('сообщение', данные_сообщения)
				...
				цепь_websocket(соединение)
					...
					.сделать ->
						сообщение = @.$.сообщение
						данные_сообщения =
							отправитель: Object.выбрать(['адресное имя', 'имя', '_id'], пользователь)
			...
			выход = ->
				...
				цепь(websocket)
					...
					.сделать () ->
						соединение.broadcast.emit('offline', Object.выбрать_поля(['_id'], пользователь))
...
http.get '/болталка/сообщения', (ввод, вывод) ->
	...
	цепь(вывод)
		...
		.все_вместе (сообщение) ->
			пользовательское.взять(сообщение.отправитель, @)
	
		.сделать (отправители) ->
			@.$.сообщения.forEach (сообщение, индекс) ->
				сообщение.отправитель = Object.выбрать(['имя', 'адресное имя', 'пол', '_id'], отправители[индекс])
			@()
		...
		.сделать (ещё_сообщения) ->
			...
			вывод.send { сообщения: @.$.сообщения, 'есть ещё?': есть_ли_ещё }

начиная_с_какого_выбрать = (ввод, возврат) ->
	if ввод.настройки.с?
		return возврат.done({ с: хранилище.collection('chat').id(ввод.настройки.с), прихватить_границу: no, откуда: 'раньше' })
