...
http.get '/болталка/сообщения', (ввод, вывод) ->
	...
	цепь(вывод)
		...
		.сделать (сообщения) ->
			return @() if сообщения.length == 0
			
			оператор_id = '$lt'
			if @.переменная('с_какого_выбрать').после?
				оператор_id = '$gt'
			
			хранилище.collection('chat').findOne({ _id: { оператор_id: сообщения[0]._id } }, @)
...
начиная_с_какого_выбрать = (ввод, возврат) ->
	if ввод.настройки.после?
		return @.done({ с: хранилище.collection('chat').id(ввод.настройки.после), прихватить_границу: no, откуда: 'раньше' })

	с_какого_выбрать = ввод.session.data['последнее сообщение в болталке']
	return @.done({ с: с_какого_выбрать, ограничение: Max_batch_size, откуда: 'позже' }) if с_какого_выбрать?
