...
http.get '/пользовательские_данные_для_страницы', (ввод, вывод) ->
	...
	if ввод.session?
		...
		данные_для_страницы.пользователь = Object.выбрать(['_id', 'имя', 'адресное имя', 'загружен ли аватар?'], пользователь)
	else
		if ввод.cookies.user?
			вывод.clearCookie 'user'
			данные_для_страницы.ошибка = 'Пользователь не найден'
	
	данные_для_страницы.пользователь.беседы = {}
	данные_для_страницы.пользователь.обсуждения = {}
	данные_для_страницы.пользователь.новости = {}
...
http.post '/человек/сменить данные', (ввод, вывод) ->
	...
	_id = пользователь._id #хранилище.collection('people').id(пользователь._id)
	цепь(вывод)
		#.сделать ->
		#	хранилище.collection('people').findOne({ _id: _id }, @)
		.сделать ->
			#if not человек?
			#	return @.error "Человек № #{_id} не найден"
...
http.post '/человек/сменить картинку', (ввод, вывод) ->
	...
	место = Options.Upload_server.File_path + '/люди/' + пользователь['адресное имя'] + '/картинка'
	...
	цепь(вывод)
		...
		.сделать ->
			хранилище.collection('people').update({ _id: пользователь._id }, { $set: { 'загружен ли аватар?': yes } }, @)
		.сделать ->
			вывод.send {}
...
http.post '/пользователь/настройки', (ввод, вывод) ->
	...
	пользователь = ввод.session.пользователь
	почта_изменилась = (пользователь.почта != ввод.body.почта)
	
	новые_данные_пользователя = {}
	
	if (ввод.body.почта)
		новые_данные_пользователя.почта = ввод.body.почта
	
	_id = пользователь._id
	цепь(вывод)
		.сделать ->
			хранилище.collection('people').update({ _id: _id }, { $set: новые_данные_пользователя }, @)
		.сделать ->
			if почта_изменилась
				global.почта.письмо(кому: пользователь.имя + ' <' + пользователь.почта + '>', тема: 'Проверка вашего нового почтового ящика', сообщение: 'Теперь это ваш почтовый ящик в нашей сети')		
			вывод.send {}
...
http.get '/пользователь/круги', (ввод, вывод) ->
	...
	цепь(вывод)
		...
		.все_вместе (_id) ->
			хранилище.collection('people').findOne({ _id: хранилище.collection('people').id(_id) }, @)
		.сделать (люди) ->
			for человек in люди
