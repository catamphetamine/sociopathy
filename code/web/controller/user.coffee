...
http.post '/вход', (ввод, вывод) ->
	...
	пользователь = пользовательское.взять.await({ имя: ввод.body.имя }, { полностью: yes })
...
http.get '/приглашение/проверить', (ввод, вывод) ->
	приглашение = db('invites')._.find_one {ключ: ввод.настройки.приглашение.toString() }
...
http.put '/сеть/человек/данные', (ввод, вывод, пользователь) ->
	if ввод.body.имя != пользователь.имя
		человек_с_таким_именем = db('people')._.find_one({ имя: ввод.body.имя, _id: { $ne: пользователь._id } })
		if человек_с_таким_именем?
			show_error('Такое имя уже занято')
		
	# проверка на занятость имени не атомарна, но пока так сойдёт
	db('people')._.update({ _id: пользователь._id }, { $set: { имя: ввод.body.имя, описание: ввод.body.описание, откуда: ввод.body.откуда, 'о себе': JSON.parse(ввод.body.о_себе) } })
...
http.post '/сеть/пользователь/настройки', (ввод, вывод, пользователь) ->
	...
	if почта_изменилась
		человек_с_такой_почтой = db('people')._.find_one({ почта: ввод.body.почта, _id: { $ne: пользователь._id } })
...
http.delete '/сеть/подсказка', (ввод, вывод, пользователь) ->
	db('people_sessions')._.update({ пользователь: пользователь._id }, { $addToSet: { 'не_показывать_подсказки': ввод.body.подсказка } })			
...
http.get '/сеть/пароль', (ввод, вывод, пользователь) ->
	db('people')._.update({ _id: пользователь._id, пароль: '123' }, { $set: { пароль: ввод.настройки.пароль } })
	вывод.send({})
