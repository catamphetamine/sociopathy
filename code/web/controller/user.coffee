...
http.get '/пользовательские_данные_для_страницы', (ввод, вывод) ->
	if ввод.session?
		цепь(вывод)
			...
			.сделать (пользователь) ->
				пользователь = Object.выбрать(['_id', 'имя', 'адресное имя', 'загружен ли аватар?', 'управляющий'], пользователь)
				пользователь.беседы = {}
...
http.put '/сеть/пользователь/круги/состав', (ввод, вывод) ->
	return if пользовательское.требуется_вход(ввод, вывод)
...
http['delete'] '/сеть/пользователь/круги/состав', (ввод, вывод) ->
	return if пользовательское.требуется_вход(ввод, вывод)
...
http.get '/сеть/пользователь/круги', (ввод, вывод) ->
	return if пользовательское.требуется_вход(ввод, вывод)
