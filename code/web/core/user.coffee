...
http.post '/вход', (ввод, вывод) ->
	...
	if пользователь.пароль != ввод.данные.пароль
		...
		if температура < The_hottest_allowed_temperature
			...
			db('people_sessions').update({ пользователь: пользователь._id }, { $set: { последний_неудавшийся_вход: { когда: new Date(), температура: температура } } })
...
http.put '/прописать', (ввод, вывод) ->
	if Options.Invites
		...
		invite = collection._.findAndModify.fiberize(collection)(query, [], update, {})
...
http.get '/приглашение/проверить', (ввод, вывод) ->
	приглашение = db('invites').get(ключ: ввод.данные.приглашение.toString())
...
http.put '/сеть/человек/данные', (ввод, вывод, пользователь) ->
	if ввод.данные.имя != пользователь.имя
		человек_с_таким_именем = db('people').find_just_one({ имя: ввод.данные.имя, _id: { $ne: пользователь._id } })
		if человек_с_таким_именем?
			throw 'Такое имя уже занято'
		
	# проверка на занятость имени не атомарна, но пока так сойдёт
	db('people').update({ _id: пользователь._id }, { $set: { имя: ввод.данные.имя, описание: ввод.данные.описание, откуда: ввод.данные.откуда, 'о себе': JSON.parse(ввод.данные.о_себе) } })
...
http.put '/сеть/человек/картинка', (ввод, вывод, пользователь) ->
	...
	пользователь = db('people').get(пользователь._id)
...
http.put '/сеть/человек/фотография', (ввод, вывод, пользователь) ->
	...
	db('people').update({ _id: пользователь._id }, { $inc: { 'photo_version': 1 } })
	вывод.send {}

http.get '/сеть/мусорка/личная', (ввод, вывод, пользователь) ->
	содержимое = db('trash').find({ пользователь: пользователь._id })
	вывод.send(содержимое: содержимое)
			
http.delete '/сеть/подсказка', (ввод, вывод, пользователь) ->
	db('people_sessions').update({ пользователь: пользователь._id }, { $addToSet: { 'не_показывать_подсказки': ввод.данные.подсказка } })			
...
http.get '/сеть/черновик', (ввод, вывод, пользователь) ->
	...
	черновик = db('drafts').get(Object.x_over_y({ пользователь: пользователь._id }, query))
...
http.get '/сеть/пароль', (ввод, вывод, пользователь) ->
	db('people').update({ _id: пользователь._id, пароль: '123' }, { $set: { пароль: ввод.данные.пароль } })
	вывод.send({})

http.put '/сеть/пользователь/язык', (ввод, вывод, пользователь) ->
	db('people').update({ _id: пользователь._id }, { $set: { язык: ввод.данные.язык } })
	вывод.send({})