...
http.put '/сеть/человек/картинка', (ввод, вывод, пользователь) ->
	options =
		...
		место: '/люди/' + пользователь['адресное имя'].to_unix_file_name() + '/картинка'
		название: 'большая'
	...
	эфир.отправить('пользователь', 'аватар обновлён', { version: пользователь.avatar_version, пользователь: пользователь['адресное имя'] }, { кому: пользователь._id })
...
http.put '/сеть/человек/фотография', (ввод, вывод, пользователь) ->
	options = 
		временное_название: ввод.данные.имя
		место: '/люди/' + пользователь['адресное имя'].to_unix_file_name()
		название: 'фотография'
