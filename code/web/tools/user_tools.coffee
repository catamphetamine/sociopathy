			...
			for куда_подставить in куда
				if куда_подставить[переменная] instanceof Array
					куда_подставить[переменная].forEach (_id, индекс) ->
						куда_подставить[переменная][индекс] = Object.выбрать(['имя', 'адресное имя', 'пол', '_id'], пользователи[_id + ''])
				else
					куда_подставить[переменная] = Object.выбрать(['имя', 'адресное имя', 'пол', '_id'], пользователи[куда_подставить[переменная] + ''])
			@.done()