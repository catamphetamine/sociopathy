...
class Цепь
	...
	on_error: (ошибка, номер_действия) ->
		...
		if ловушка?
			...
			if result != no
				@остановлена = yes
				return
			@отменить_действия_вплоть_до = номер_действия
			return no
		else
			#console.error "Error:"
			#console.error ошибка
	...
	возврат: (откуда, из_какого_поддействия) =>
		функция_возврата = (ошибка) =>
			...
			if ошибка?
				if @debug
					console.log "Действие не было выполнено из-за ошибки."
					if действие.номер?
						console.log "Номер действия: #{действие.номер}"
				if @on_error(ошибка, откуда) != no
					return
				console.log "Ошибка проигнорирована"
			...
			if @отменить_действия_вплоть_до?
				if действие.номер?
					if действие.номер <= @отменить_действия_вплоть_до?
						if @debug
							console.log "Результаты выполнения действия отменяются из-за ошибки, произошедшей ранее."
						return
	...
	дальше: (переменные) ->
		...
		if @debug
			console.log '==================='
			if действие?
				if действие.номер? && действие.номер > 1
