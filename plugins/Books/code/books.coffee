...
http.get '/сеть/книги/поиск', (ввод, вывод) ->
	...
	книги = db('books').find({ название: { $regex: шаблон, $options: 'i' } }, { limit: ввод.данные.max })
...
http.post '/сеть/книги', (ввод, вывод, пользователь) ->
	...
	for книга in книги.удалённые
		...
		у_кого = db('peoples_books').get({ книги: { $in: [_id] } })
		
		if у_кого?
			держатель = пользовательское.взять.do(у_кого.пользователь)
			книга = db('books').get(_id)
	...
	for книга in книги.обновлённые_обложки
		...
		db('books').update({ _id: _id }, { $inc: { 'обложка.version': 1 }, $set: update })

	# переименовать переименованные	книги
		
	for книга in книги.переименованные
		_id = db('books').id(книга._id)
		db('books').update({ _id: _id }, { $set: { название: книга.название, сочинитель: книга.сочинитель } })
