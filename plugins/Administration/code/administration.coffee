...
http.get '/сеть/управление/хранилище/изменить', (ввод, вывод, пользователь) ->
	...
	db('people').dropIndex.bind_await(db('people'))({ 'адресное имя': 1 })
		

	people = db('people')._.find()
	for man in people
		db('people')._.update({ _id: man._id }, { $set: { id: man['адресное имя'] }})
		db('people')._.update({ _id: man._id }, { $unset: { 'адресное имя': yes }})
	
	db('people')._.index('id', yes)
