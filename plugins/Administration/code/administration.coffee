...
http.post '/сеть/управление/приглашение/выдать', (ввод, вывод, пользователь) ->
	...
	приглашение = db('invites').add({ ключ: ключ, пользователь: пользователь._id })
...
http.post '/сеть/управление/хранилище/изменить', (ввод, вывод, пользователь) ->
	...
	человек = db('people').get(имя: 'Дождь со Снегом')
	второй_человек = db('people').get(имя: 'Анна Каренина')
